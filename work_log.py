...
def task_entry():
    ...
    with open('tasks.csv', 'a+', newline='') as taskfile:
        ...
        writer.writerow({'name': name, 'time': time, 'notes': notes, 'date': date})
...
def date_input(date_wrapper):
    ...
    while len(date_wrapper) == 0:
        try:
            input_string = input('>>>')
            date_wrapper.append(datetime.datetime.strptime(input_string, '%m/%d/%Y'))
        except ValueError:
            print('Date must be valid and in format MM/DD/YYYY. Try again.')

    
def date_find():
    '''Searches for all entries matching a specific date or falling within a date range'''
...
or 2 to search within a date range.\n>>>''').strip()
        if search_mode not in ['1', '2']:
            ...
            search_mode = None
    print('Input {}date in format MM/DD/YYYY'.
          format('start ' if search_mode == '2' else ''))
    ...
    if search_mode == '2':
        ...
        while not search_date_2:
            ...
            if search_date_1[0].timestamp() > search_date_2[0].timestamp():
                print('First date cannot be later than second date. Enter end date again.')
                search_date_2 = []
    #run search
    search_results = []
    with open('tasks.csv') as taskfile:
        ...
        if search_mode == '1':
            for row in reader:
                if datetime.datetime.strptime(row['date'], '%m/%d/%Y') == search_date_1[0]:
                    search_results.append(row)
        else:
            for row in reader:
                if ((datetime.datetime.strptime(row['date'], '%m/%d/%Y').timestamp() 
                    >= search_date_1[0].timestamp()) 
                and (datetime.datetime.strptime(row['date'], '%m/%d/%Y').timestamp() 
...
def time_find():
    '''Searches for all entries matching a specific time spent in minutes'''
    search_time = None
    search_results = []
    #get search time as input
...
def exact_find():
    '''Finds string within names or notes of entries'''
    search_string = None
    search_results = []
    #get search string
    print('Enter text to be searched')
    while search_string == None or search_string.isspace():
        search_string = input('>>>')
    ...
    with open('tasks.csv') as taskfile:
        reader = csv.DictReader(taskfile)
        for row in reader:
            if (row['name'].find(search_string) > -1 
                or row['notes'].find(search_string) > -1):
                search_results.append(row)
    if len(search_results) == 0:
        print('\nNo results found.\n')
        task_search()
...
def regex_find():
    '''Searches entries by regular expression'''
    search_exp = None
    search_results = []
    #get search expression
    print("Enter a valid regular expression.")
    while search_exp == None:
        search_exp = input('>>>')
        try:
    ...
    with open('tasks.csv') as taskfile:
        reader = csv.DictReader(taskfile)
        for row in reader:
            if (re.search(search_exp, row['name']) 
                    or re.search(search_exp, row['time'])
                    or re.search(search_exp, row['notes']) 
                    or re.search(search_exp, row['date'])
                ):
                search_results.append(row)
    if len(search_results) == 0:
        print('\nNo results found.\n')
...
def display_results(results_list):
    '''Displays search results one by one, allowing each entry to be edited or deleted'''
    count = 1
    for entry in results_list:
        ...
        selection = None
        while not selection:
            selection = input('Select {}[E]dit entry, [D]elete entry, [S]earch menu \n>>>'.
                format('[N]ext result, ' if count < len(results_list) else ''))
...
def edit_entry(task_dict):
    '''Allows user to edit specific field of entry'''
...
Enter 5 to go back to search results, 6 to go back to search menu\n>>>''')
        ...
        if field == '6':
            return entry_search()
    print('The {} of the current entry is {}'.
          format(field_dict[field], task_dict[field_dict[field]]))
    #get new data to replace old
    new_info = None
    while not new_info:
        ...
        if field == '3':
            try:
                ...
                new_info = None         
    #create new file with updated information, and overwrite old file
    edited = []
    with open('tasks.csv') as taskfile:
        reader = csv.DictReader(taskfile)
        for row in reader:
...
def delete_entry(task_dict):
    ...
    #create new file with entry omitted, and overwrite old file
    edited = []
    with open('tasks.csv') as taskfile:
        reader = csv.DictReader(taskfile)
        for row in reader:
